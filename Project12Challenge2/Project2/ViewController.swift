...
class ViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		if let savedHighScore = defaults.value(forKey: "highScore") as? Int {
	...
	@IBAction func buttonTapped(_ sender: UIButton) {
		...
		if questionsAsked <= 10 {
			...
			if score > highScore {
				highScore = score
				highScoreLabel.text = String(highScore)
				title = "High Score Beat!"
				final = UIAlertController(title: title, message: "You got \(score). Congradulations!", preferredStyle: .alert)
				save()
				questionsAsked = 0
				score = 0
